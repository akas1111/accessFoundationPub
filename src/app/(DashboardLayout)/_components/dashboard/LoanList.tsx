...
export default function MyLoans({ setCanApply }: { setCanApply: () => void }) {
  ...
  const columns: GridColDef<any[number]>[] = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        if (
          ["REJECTED", "DELETED"].includes(params?.row?.loanProcessingStatus)
        ) {
          return <span>-</span>;
        } else {
          return (
    ...
    },
    {
      ...
      width: 140,
      renderCell: (params) => {
        if (["CANCELLED", "DELETED"].includes(params?.row?.status)) {
          return <span>-</span>;
        } else {
          return (
            <Chip
  ...
  return (
    <div style={{ maxWidth: "calc(100vw - 32px)" }}>
      <DashboardCard title="My Loan Applications">
        <div
        //style={{ maxWidth: "calc(100vw - 60px - 48px)" }}
        >
          <DataGrid
            // autoPageSize
            // disableAutosize
